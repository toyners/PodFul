...
type DeliveryPointCreator_UnitTests() =
    ...
    member public this.``Delivery Point data list is null so no delivery points are created.``() =
        let instances = PodFul.WPF.Miscellaneous.DeliveryPointCreator.CreateDeliveryPoints(null, new FileDeliveryLogger())
    ...
    member public this.``Delivery Point data list is empty so no delivery points are created.``() =
        ...
        let instances = PodFul.WPF.Miscellaneous.DeliveryPointCreator.CreateDeliveryPoints(deliveryPointData, new FileDeliveryLogger())
